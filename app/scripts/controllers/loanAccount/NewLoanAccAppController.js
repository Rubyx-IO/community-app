(function (module) {
  mifosX.controllers = _.extend(module, {
    NewLoanAccAppController: function (
      scope,
      routeParams,
      resourceFactory,
      location,
      ...
      scope.loanProductChange = function (loanProductId) {
        ...
        resourceFactory.clientcollateralTemplateResource.getAllCollaterals(
          { clientId: scope.clientId, prodId: loanProductId },
          function (data) {
            scope.collateralsData = data;
            scope.collateralsData = scope.collateralsData.filter(
              (collateral) => collateral.quantity != 0
            );
          }
        );
        // scope.datatables = [];
        resourceFactory.loanResource.get(scope.inparams, function (data) {
          ...
          scope.loandetails.interestValue =
            scope.loanaccountinfo.interestType.value;
          scope.loandetails.amortizationValue =
            scope.loanaccountinfo.amortizationType.value;
          scope.loandetails.interestCalculationPeriodValue =
            scope.loanaccountinfo.interestCalculationPeriodType.value;
          scope.loandetails.transactionProcessingStrategyValue =
            scope.formValue(
          ...
          angular.forEach(datatables, function (datatable, index) {
            ...
            angular.forEach(
              ...
              function (colHeader, i) {
                ...
                if (
                  datatable.columnHeaderData[i].columnDisplayType == "DATETIME"
                ) {
                  scope.formDat.datatables[index].data[
                    datatable.columnHeaderData[i].columnName
                  ] = {};
                }
              }
            );
      ...
      scope.previewClientLoanAccInfo = function () {
        ...
        scope.formData.disbursementData =
          scope.loanaccountinfo.disbursementDetails || [];
        scope.collaterals = [];
        ...
        scope.loandetails.loanTermFrequencyValue =
          scope.loanaccountinfo.termPeriodFrequencyType.value;
        ...
        scope.loandetails.repaymentFrequencyValue =
          scope.loanaccountinfo.repaymentFrequencyType.value;
        ...
        scope.formData.fixedPrincipalPercentagePerInstallment =
          scope.loanaccountinfo.fixedPrincipalPercentagePerInstallment;
        ...
        if (
          ...
          scope.loanaccountinfo.interestRecalculationData
        ...
        scope.loandetails.productName = scope.formValue(
          scope.products,
      ...
      scope.formValue = function (array, model, findattr, retAttr) {
        ...
        console.log(array, model, findattr, retAttr);
      ...
      scope.addCollateral = function () {
        scope.collateralAddedDataArray.push(
          scope.collateralsData.filter(
            (collateral) =>
              scope.collateralFormData.collateralId == collateral.collateralId
          )[0]
        );
        scope.collateralsData = scope.collateralsData.filter(
          (collateral) =>
      ...
      scope.updateValues = function () {
        scope.collateralObject = scope.collateralsData.filter(
          (collateral) =>
            collateral.collateralId == scope.collateralFormData.collateralId
        )[0];
        scope.collateralFormData.total =
          scope.collateralFormData.quantity * scope.collateralObject.basePrice;
        scope.collateralFormData.totalCollateral =
          (scope.collateralFormData.total * scope.collateralObject.pctToBase) /
          100.0;
      };

      scope.deleteCollateral = function (index) {
        scope.collateralId = scope.collaterals[index].collateralId;
        scope.collateralObject = scope.collateralAddedDataArray.filter(
          (collateral) => collateral.collateralId == scope.collateralId
        )[0];
      ...
      scope.previewRepayments = function () {
        ...
        if (
          ...
          (!_.isUndefined(scope.formData.datatables) &&
            scope.formData.datatables.length == 0)
        ) {
          ...
          angular.forEach(scope.datatables, function (datatable, index) {
            ...
            angular.forEach(scope.columnHeaders, function (colHeader, i) {
              ...
              if (scope.columnHeaders[i].columnDisplayType == "DATE") {
                ...
                scope.columnHeaders[i].columnDisplayType == "DATETIME"
              ) {
                if (
                  !_.isUndefined(
                    scope.formDat.datatables[index].data[
  ...
  mifosX.ng.application
    .controller("NewLoanAccAppController", [
      "$scope",
      "$routeParams",
      "ResourceFactory",
      "$location",
      ...
      "WizardHandler",
      "$translate",
      mifosX.controllers.NewLoanAccAppController,
    ])
    .run(function ($log) {
