(function (module) {
      ...
      resourceFactory.clientResource.get(
        ...
        function (data) {
          ...
          if (data.clientNonPersonDetails?.incorpValidityTillDate) {
            var incorpValidityTillDate = dateFilter(
              data.clientNonPersonDetails.incorpValidityTillDate,
              scope.df
            );
            scope.date.incorpValidityTillDate = new Date(
          ...
          if (data.timeline.submittedOnDate) {
            var submittedOnDate = dateFilter(
      ...
      scope.submit = function () {
        ...
        if (scope.choice === 1) {
          if (scope.date.activationDate) {
            this.formData.activationDate = dateFilter(
              scope.date.activationDate,
              scope.df
            );
          }
        }
        if (scope.date.dateOfBirth) {
          this.formData.dateOfBirth = dateFilter(
            scope.date.dateOfBirth,
            scope.df
          );
        }

        if (scope.date.submittedOnDate) {
          this.formData.submittedOnDate = dateFilter(
            scope.date.submittedOnDate,
            scope.df
          );
        }
        ...
        if (
          this.formData.legalFormId == scope.clientPersonId ||
          this.formData.legalFormId == null
        ) {
        ...
        resourceFactory.clientResource.update(
          { clientId: routeParams.id },
          this.formData,
          function (data) {
  ...
  mifosX.ng.application
    .controller("EditClientController", [
      ...
      "Upload",
      "$rootScope",
      mifosX.controllers.EditClientController,
    ])
    .run(function ($log) {
